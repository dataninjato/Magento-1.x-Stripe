...
appmerce.stripeCc = function ($) {
    ...
    $.fn.toggleInputError = function (valid) {
        if(this.val().length) {
            this.parent().toggleClass('appmerce-has-error', !valid);
        }
        return this;
    };
    ...
    self.toggleNewCard = function() {
        ...
        $inputs.savedCard.change(function () {
            ...
            $inputs.savedCard
                .parent()
                .removeClass('appmerce-label-active')
                .end()
                .filter($(this)).parent().addClass('appmerce-label-active');
    ...
    self.showLabel = function ($el) {
        $el.parents('li').toggleClass('appmerce-show_label', !!$el.val().length);
    };
    ...
    self.cardEntryListener = function () {
        ...
        if (self.validCard()) {
            $('#payment_form_stripe_cc .appmerce-has-error').removeClass('appmerce-has-error');
            if (self.newTokenRequired()) {
                self.createToken();
            } else {
                self.disableContinueBtn(false);
    ...
    self.stripeResponseHandler = function (status, response) {
        ...
        if (status !== 200 || response.error) {
            $('#stripe-cards-select-new')
                ...
                .remove()
                .end()
                .prepend('<p class="appmerce-error">'+response.error.message+'</p>');
        } else {

            //Remove any previous errors
            $('#stripe-cards-select-new .appmerce-error').remove();
